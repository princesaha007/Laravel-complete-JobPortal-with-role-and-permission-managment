            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left font-medium text-gray-600 uppercase">Permissions</th>
                ...
                @foreach($roles as $role)
                    ...
                    <tr>
                        <td class="px-4 py-3 text-gray-900 font-semibold">{{$role->id}}</td>
                        <td class="px-4 py-3 text-gray-700">{{$role->name}}</td>
                        <td>{{$role->permissions->pluck('name')->implode(', ')}}</td>
                        <td class="px-4 py-3 text-gray-600">{{\Carbon\Carbon::parse($role->created_at)->format('d M Y')}}</td>
                        ...
                        <div class="flex flex-wrap gap-2">
                                ...
                                <!-- </a> -->
                                 ...
                                 @can('edit roles')
                                <a href= "{{route('roles.edit', $role->id)}}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-xs inline-block">
                                    Update
                                </a>
                                @endcan

                                @can('delete roles')
                                <a href="{{route('roles.destroy', $role->id)}}" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs inline-block">
                                    Delete
                                </a>
                                 @endcan
